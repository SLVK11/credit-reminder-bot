...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text("Привет! Напиши команду /дата в формате дд.мм.гггг, чтобы узнать крайний срок возврата.")

async def date_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        if len(context.args) != 1:
            raise ValueError("Пример: /дата 23.04.2025")
...
if __name__ == '__main__':
    ...
    app.add_handler(CommandHandler("дата", date_handler))
    app.run_polling()
